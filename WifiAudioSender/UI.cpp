...
void loadControls(HWND hWnd){
	...
	txtIP		= CreateWindowEx(WS_EX_CLIENTEDGE, WC_EDIT, _T(""), WS_TABSTOP | WS_VISIBLE | WS_CHILD | ES_LEFT,rctIP.left, rctIP.top, rctIP.right,rctIP.bottom, hWnd, (HMENU)ID_CONTROL_IP, hInst, NULL);
...
void fillCombo(){
	...
	SendMessage(comboFrom, (UINT)CB_SETCURSEL, 0, 0);
	SendMessage(comboTo, (UINT)CB_SETCURSEL, 0, 0);
Exit:
	SAFE_RELEASE(pDevice);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	switch (message) {
		...
		case WM_COMMAND:{
			...
			switch (wmId) {
				...
				case ID_FEATURE_START:{
					...
					WideCharToMultiByte(CP_ACP, 0, str, lstrlen(str), sIp, sizeof(sIp), NULL, NULL);
					int posFrom = SendMessage(comboFrom, (UINT)CB_GETCURSEL, 0, 0);
					int posTo = SendMessage(comboTo, (UINT)CB_GETCURSEL, 0, 0);
					if (ap.StartSending(sIp, dev[0][posFrom], dev[0][posTo]) == ERROR_SUCCESS){
		...
		case WM_DESTROY:{
			...
			wsprintf(wcsrchr(app, L'.') + 1, L"ini");
			GetWindowText(txtIP, str, 255);
			WritePrivateProfileString(L"WAS", L"from", dev[0][SendMessage(comboFrom, (UINT)CB_GETCURSEL, 0, 0)], app);
			WritePrivateProfileString(L"WAS", L"to", dev[0][SendMessage(comboTo, (UINT)CB_GETCURSEL, 0, 0)], app);
			WritePrivateProfileString(L"WAS", L"IP", str, app);
			WritePrivateProfileString(L"WAS", L"started", IsWindowEnabled(btnStart) ? L"0" : L"1", app);
